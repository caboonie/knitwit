...
def add_pattern(user, pattern_json):
    pattern = Pattern(user_id = user.id, pattern_json = pattern_json)
    session.add(pattern)
    session.commit()
...
def get_users_patterns(id_num):
    return session.query(Pattern).filter_by(user_id=id_num).all()
